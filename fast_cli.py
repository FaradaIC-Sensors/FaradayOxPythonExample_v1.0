...
from client import (
    ...
    build_registers_read_measurement_registers,
...
def cli_app(port: str = "COM5", *, measure_sht40: bool = True, measure_oxygen: bool = True):
    ...
    if measure_oxygen:
        ...
        time.sleep(0.2)  # longer wait if needed for gas measurement
        request_and_log_measurement_registers(module, port, header="After O2 measurement")
...
def request_and_log_measurement_registers(module, port, header=None):
    status, frame = send_frame(port, build_registers_read_measurement_registers(), OPERATION_READ)
    if not status:
        return
    frame_data = process_frame(frame)
    if not frame_data:
    ...
    # Pad partial measurement frame (0x06–0x13) into a full page
    full_page = [0] * REGISTERS_PAGE_SIZE
    start = int(Registers.REG_STATUS)  # 0x06
    full_page[start:start + len(frame_data)] = frame_data

    if not module.deserialize(full_page):
        return
